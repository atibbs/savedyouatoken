/**
 * The default command: `savedyouatoken <file...> [options]` and `savedyouatoken models`.
 *
 * This is a straight port of the CLI's original (pre-subcommand) behavior. Its flags, exit
 * codes, `--json` shape, and `--contract-json` shape are unchanged — see docs/contracts.md's
 * existing-format inventory and `verify-packed-cli.mjs`, which asserts the legacy `--json` shape
 * has no `contract` field. New functionality lives in the other `discover`/`baseline`/`compare`/
 * `policy` commands instead of changing this one.
 */
...
import { VERSION, c, fail } from '../support';
...
export const HELP = `savedyouatoken ${VERSION} — find the waste in your LLM prompts

USAGE
  savedyouatoken <file...> [options]
  savedyouatoken models
  savedyouatoken discover [roots...] [options]
  savedyouatoken baseline create <file>|--from-report <path> [options]
  savedyouatoken compare <file>|--from-report <path> --baseline <path> [options]
...
export function runAudit(argv: string[]): void {
  const { files, options, command } = parseArgs(argv);
