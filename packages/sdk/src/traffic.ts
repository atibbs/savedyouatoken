import type { Workload, WorkloadOverrides } from './types';
...
export class TrafficWindow {
  ...
  private measuredRequestsPerDay(now: number): { value: number; matured: boolean } {
    this.prune(now);
    const n = this.obs.length;
    if (n === 0) return { value: 0, matured: false };
    const earliest = this.obs[0]!.t;
    const spanMs = now - earliest;
    // A handful of requests spanning a few milliseconds cannot be annualised honestly.
    if (n < this.config.minObservationsForMaturity || spanMs < this.config.minSpanMsForMaturity) {
      return { value: 0, matured: false };
    }
    const spanDays = spanMs / DAY_MS;
    return { value: n / spanDays, matured: true };
  }
  ...
  measure(now: number): MeasuredWorkload {
    ...
    const requestsPerDay = o.requestsPerDay ?? (rate.matured ? rate.value : DEFAULT_REQUESTS_PER_DAY);
    const matured = o.requestsPerDay != null ? true : rate.matured;
