...
- [ ] 3.1 Implement the shape key (hash of model + system + tools) and a bounded LRU dedupe + sampling policy (analyse once per shape, re-trigger on change)
- [ ] 3.2 Implement the traffic model (rolling window → requests/day, average output tokens, cache-hit rate) mapping to core's `Workload`, with explicit overrides
- [ ] 3.3 Implement `createAuditor(adapter, opts)` with `observe(params, response)` that bridges a captured shape to core `analyze()` using an injected `gpt-tokenizer` counter — never reimplementing analysis

## 4. Result delivery (sinks)

- [ ] 4.1 Define the `AuditSink` interface and implement `consoleSink`, `fileSink`, and a `callback` sink
- [ ] 4.2 Implement the opt-in network `dashboardSink` that transmits only `toSharedReport(result)` (prompt-free); wire the default (console in dev, no-op in prod)

## 5. Ergonomic wrappers

- [ ] 5.1 Implement `wrapAnthropic(client)` / `wrapOpenAI(client)`: passthrough proxy that awaits the real call, returns it unchanged, then schedules `observe()` off the hot path (microtask, errors swallowed)
...
- [ ] 6.2 Dedupe/sampling: a repeated identical shape analyses at most per the sampling policy; a changed shape re-triggers
- [ ] 6.3 Parity: the SDK path yields the same findings/counts/cost as calling core `analyze()` directly on the same input
- [ ] 6.4 Privacy: the `dashboardSink` payload contains no prompt or tool text (assert against the serialised report)
