...
- [x] 2.1 Add `.github/workflows/release.yml` triggered on **push to `main`**: when `packages/cli`'s version is ahead of the version on npm, build and publish — so a merged bump ships automatically, with no manual tag step
- [x] 2.2 Publish to a **candidate dist-tag** (e.g. `rc`), never directly to `latest`; authenticate with **GitHub Actions trusted publishing (OIDC, `id-token: write`)** and provenance — no long-lived `NPM_TOKEN`
- [x] 2.3 Document the one-time bootstrap: claim the `savedyouatoken` name with a short-lived token, configure the repo/workflow as a trusted publisher, then revoke the bootstrap token
...
- [x] 4.1 Add a job that installs `savedyouatoken@<candidate>` from the registry in a clean environment (no repo on PATH) and asserts it executes and prints its published version
- [x] 4.2 Promote the candidate to `latest` (`npm dist-tag add`) only if that check passes; on failure, fail the release and leave `latest` unchanged
